...
(function($) {
  ...
  function getCurrentHash() {
	...
	------------------------------------------------------ */ 
  	...
  	if($("html").hasClass('ie8')) {
  		$("#home").backstretch("images/hero-bg.jpg");  	
  ...
  toggle_button.on("click", function(e) {
	...
	/* Smooth Scrolling
	------------------------------------------------------ */
  function smoothScroll(selector, callback) {
    var $target = $(selector);
  ...
  $('.smoothscroll').on('click', function (e) {
   	var $this = $(this),
      target = $this.data("hash") || this.hash,
    	$target = $(target),
      enablePush = Modernizr.history && !$this.hasClass("hash") && $("body").hasClass('homepage'),
      pushUrl = $this.attr("href");

    if (enablePush && pushUrl === "#home")
      pushUrl = "/";
    else if (pushUrl && (pushUrl[pushUrl.length - 1] !== "/"))
      pushUrl = pushUrl + "/";
    ...
    smoothScroll(target, function () {
      if (enablePush) {
        if (location.pathname !== pushUrl) {
          window.history.pushState({hash: target}, "", pushUrl);
          currentHash = target;
        }
      } else {
  ...
  if (Modernizr.history) {
    $(window).on("popstate", function(e) {
      var state = e.originalEvent.state,
        hash = state ? state.hash : getCurrentHash();
      smoothScroll(hash);
    });
  }
	...
	if($("body").hasClass('homepage')) {
	    ...
	    handler: function(event, direction) {
			  ...
			  var active_section = $(this);
				...
				var active_link = $('#nav-wrap a[data-hash="#' + active_section.attr("id") + '"]');
